...
if __name__ == "__main__":
    ...
    a = brats_transforms({'image':'D:\\Datasets\\BraTS2020\\BraTS2020_ValidationData\\MICCAI_BraTS2020_ValidationData\\BraTS20_Validation_003\\BraTS20_Validation_003_t1.nii'})['image']
    a = a.to(device)
    a = a.unsqueeze(0)
    with torch.no_grad():
        with model.ema_scope():
            PILImage.fromarray(a[0, 0, :, :, 30].cpu().detach().numpy()*200.).show()
            h = model.encoder(a)
