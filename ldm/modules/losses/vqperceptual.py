...
def adopt_weight(weight, global_step, threshold=0, value=0., step_each=1):
    if global_step < threshold or global_step % step_each != 0:
        weight = value
    return weight
...
class VQLPIPS3DWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0, g_weight = 1.0,
                 perceptual_weight=1.0, use_actnorm=False, disc_conditional=False,
                 disc_ndf=64, disc_loss="hinge", n_classes=None, perceptual_loss="lpips",
                 pixel_loss="l1", disc_each_step=1):
        super().__init__()
        assert disc_loss in ["hinge", "vanilla"]
        ...
        self.discriminator_weight = disc_weight
        self.disc_conditional = disc_conditional
        self.generator_weight = g_weight
        self.n_classes = n_classes
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer=None):
        ...
        d_weight = torch.norm(nll_grads) / (torch.norm(g_grads) + 1e-4)
        d_weight = torch.clamp(d_weight, 0.0, 1e4).detach()
        d_weight = d_weight * self.generator_weight
        return d_weight
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        if self.perceptual_weight > 0:
            ...
            hcwd_p_loss = self.perceptual_loss(inputs_hcwd.contiguous(), reconstructions_hcwd.contiguous())
            wchd_p_loss = self.perceptual_loss(inputs_wchd.contiguous(), reconstructions_wchd.contiguous())
            dchw_p_loss = self.perceptual_loss(inputs_dchw.contiguous(), reconstructions_dchw.contiguous())

            p_loss_squared = torch.cat([hcwd_p_loss, wchd_p_loss, dchw_p_loss]) ** 2
            p_loss = p_loss_squared.mean()
            rec_loss = rec_loss + self.perceptual_weight * p_loss
        else:
            p_loss = torch.tensor([0.0])
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = adopt_weight(self.disc_factor, global_step, threshold=self.discriminator_iter_start)
            loss = nll_loss + g_weight * disc_factor * g_loss + self.codebook_weight * codebook_loss.mean()
            ...
            log = {"{}/total_loss".format(split): loss.clone().detach().mean(),
                   ...
                   "{}/rec_loss".format(split): rec_loss.detach().mean(),
                   "{}/p_loss".format(split): p_loss.detach().mean(),
                   "{}/g_weight".format(split): g_weight.detach(),
                #    "{}/disc_factor".format(split): torch.tensor(disc_factor),
                   "{}/g_loss".format(split): g_loss.detach().mean(),
                   }
            if predicted_indices is not None:
